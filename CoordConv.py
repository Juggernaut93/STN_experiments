"""
PyTorch implementation of the CoordConv layer.
=====================================
Original author: Muhammed Kocabas <https://github.com/mkocabas/CoordConv-pytorch>
"""
import torch
...
'''
An alternative implementation for PyTorch with auto-inferring the x-y dimensions.
'''
class AddCoords(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        if self.with_r:
            rr = torch.sqrt(torch.pow(xx_channel.type_as(input_tensor), 2) + torch.pow(yy_channel.type_as(input_tensor), 2))
            ret = torch.cat([ret, rr], dim=1)
